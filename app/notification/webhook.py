...
async def send_notification(notif: Notification) -> bool:
    ...
    result = await send_req(w_address=WEBHOOK_ADDRESS, data=jsonable_encoder(notif))
    return result
...
async def send_req(w_address: str, data):
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(w_address, json=data, headers=headers) as r:
                if r.status == 200:
                    return True
                logger.error(f"Failed to send data to {w_address}: {r.status} {await r.text()}")
